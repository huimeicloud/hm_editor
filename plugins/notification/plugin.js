...
CKEDITOR.plugins.add( 'notification', {
	...
	init: function( editor ) {
		...
		editor.showNotification = function (message, type, progressOrDuration, autoClose) {
			if(window['HmGrowl'] || parent['HmGrowl']) {
                var HmGrowl = window['HmGrowl'] || parent['HmGrowl'];
				var timeout = Number(progressOrDuration) || 0; // timeout=0 时提示不自动关闭.
				if (!autoClose && !progressOrDuration) {
					timeout = type === 'error' ? 5000 : 1000;
				}
				switch (type) {
					case 'warning':
						HmGrowl.warn(message, timeout);
						break;
					case 'error':
						HmGrowl.error(message, timeout);
						break;
					case 'success':
						HmGrowl.success(message, timeout);
						break;
					case 'show':
						return HmGrowl.show(message, timeout);
					default:
						HmGrowl.info(message, timeout);
						break;
				}
				return null;
            }
		...
		editor.on( 'key', function handleEsc( evt ) {
