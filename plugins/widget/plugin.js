...
( function() {
	...
	NestedEditable.prototype = CKEDITOR.tools.extend( CKEDITOR.tools.prototypedCopy( CKEDITOR.dom.element.prototype ), {
		...
		setData: function( data ) {
			...
			//会把 br 从 span里面提出来, 提到父级的 p 里面? 但是如果父级是 span 的话就会有 bug...
	...
	function cleanUpAllWidgetElements( widgetsRepo, container ) {
		...
		for ( ; i < l; ++i ) {
			...
			if ( element.type == CKEDITOR.NODE_ELEMENT && element.data( 'widget' ) ) {
				var pageBreakClasses = [];
				if (CKEDITOR.plugins.pagebreakCmd) {
					// 取出分页标记
					var classes = wrapper.$.className.split(' ');
					for (var _ = 0; _ < classes.length; _++) {
						if (classes[_].indexOf(CKEDITOR.plugins.pagebreakCmd.CONNECTOR_CLASS_NAME) === 0) {
							pageBreakClasses.push(classes[_]);
						}
					}
				}
				element.replace( wrapper );
				widgetsRepo.wrapElement( element );
				if (pageBreakClasses.length) {
					$(element.$.parentNode).addClass(pageBreakClasses.join(' '));
				}
			} else {
	...
	function setupKeyboardObserver( widgetsRepo ) {
		...
		editor.on( 'key', function keyboardObserver1( evt ) {
	...
	function setupSelectionObserver( widgetsRepo ) {
		...
		editor.on( 'selectionChange', function selectAndFocusWidgetAndEditable( evt ) {
		...
		editor.on( 'dataReady', function disselectAndBlurWidgets() {
	...
	function setupWidgetsLifecycleEnd( widgetsRepo ) {
		...
		editor.on( 'toDataFormat', function( evt ) {
			...
			evt.data.dataValue.forEach( function( element ) {
				...
				// Nested editable.
				else if ('data-cke-widget-editable' in attrs && toBeDowncasted.length > 0) {
	...
	function setupWidgetsLifecycleStart( widgetsRepo ) {
		...
		editor.on( 'dataReady', function manageWidgetsElements() {
